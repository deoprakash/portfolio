export const typingItems = [
  'LLM Applications',
  'Multi-Agent Systems',
  'Machine Learning Pipelines',
  'Computer Vision Systems',
  'AI Automation Platforms',
]

export const heroSummary =
  'Deo Prakash is an AI Engineer and Software Developer specializing in end-to-end Artificial Intelligence systems, combining backend engineering with applied Machine Learning and Generative AI. Experienced in building production-grade LLM applications, multi-agent orchestration platforms, RAG pipelines, and computer vision systems, delivering measurable business impact.'

export const aboutSummary =
  'AI Engineer and Software Developer specializing in end-to-end AI systems, combining strong backend engineering with applied Machine Learning and Generative AI. Proven experience building production-grade LLM applications, multi-agent orchestration platforms, RAG pipelines, and computer vision systems.'

export const contactInfo = {
  phone: '+91 6203608218',
  email: 'deoprakash364@gmail.com',
  linkedin: 'https://linkedin.com/in/deo-prakash-152265225',
  github: 'https://github.com/deoprakash',
}

export const stats = [
  { label: 'Projects Built', value: 4 },
  { label: 'Research Publications', value: 2 },
  { label: 'AI Systems Deployed', value: 6 },
  { label: 'Years of Development', value: 3 },
]

export const education = {
  degree: 'Bachelor of Technology (B.Tech)',
  field: 'Computer Science Engineering - Artificial Intelligence',
  school: 'Shri Shankaracharya Institute of Professional Management & Technology',
  location: 'Raipur, Chhattisgarh',
  period: '2022 - 2026',
  cgpa: '8.15',
}

export const publication = {
  title: 'Smart Factory Product Defect Detection Using Deep Learning',
  publisher: 'Springer Nature',
  year: '2025',
  doi: 'https://doi.org/10.1007/978-981-95-0063-5_34',
}

export const skills = [
  {
    title: 'Languages',
    items: [
      { name: 'Python (Advanced)', level: 95 },
      { name: 'JavaScript', level: 85 },
      { name: 'SQL', level: 80 },
      { name: 'HTML/CSS', level: 80 },
    ],
  },
  {
    title: 'Frameworks',
    items: [
      { name: 'LangChain', level: 85 },
      { name: 'LangGraph', level: 82 },
      { name: 'TensorFlow', level: 80 },
      { name: 'Keras', level: 78 },
      { name: 'Scikit-learn', level: 86 },
      { name: 'OpenCV', level: 82 },
      { name: 'FastAPI', level: 84 },
      { name: 'Flask', level: 75 },
      { name: 'React.js', level: 83 },
      { name: 'React Native', level: 76 },
    ],
  },
  {
    title: 'AI / ML',
    items: [
      { name: 'Machine Learning', level: 92 },
      { name: 'Deep Learning (CNNs)', level: 88 },
      { name: 'Natural Language Processing', level: 80 },
      { name: 'Generative AI', level: 86 },
      { name: 'RAG', level: 84 },
      { name: 'AI Agents', level: 82 },
      { name: 'Transfer Learning', level: 80 },
      { name: 'Sentiment Analysis', level: 75 },
      { name: 'Text Classification', level: 76 },
      { name: 'Hyperparameter Tuning', level: 74 },
    ],
  },
  {
    title: 'Tools',
    items: [
      { name: 'Docker', level: 82 },
      { name: 'Git', level: 86 },
      { name: 'CI/CD (GitHub Actions)', level: 78 },
      { name: 'MLOps', level: 74 },
      { name: 'REST Architecture', level: 84 },
      { name: 'Android Studio', level: 72 },
      { name: 'Google Colab', level: 70 },
    ],
  },
  {
    title: 'Data & Databases',
    items: [
      { name: 'Pinecone', level: 80 },
      { name: 'Chroma', level: 78 },
      { name: 'MySQL', level: 78 },
      { name: 'MongoDB', level: 82 },
      { name: 'PostgreSQL', level: 80 },
      { name: 'Power BI', level: 70 },
      { name: 'Google BigQuery', level: 72 },
    ],
  },
]

export const experiences = [
  {
    role: 'Associate Software Developer',
    company: 'Grid Seven AI LLP (Remote)',
    period: 'Oct 2025 - Present',
    bullets: [
      'Engineered an automated OKR analysis system using time series forecasting and MongoDB.',
      'Improved tracking efficiency by 20% via a React dashboard.',
      'Architected a modular cross-platform desktop application, reducing feature integration effort by 15%.',
      'Built REST APIs to support analytics and AI inference workflows.',
    ],
  },
  {
    role: 'AI / ML Consultant',
    company: 'RTS Consulting Automation (Canada, Remote)',
    period: 'Jul 2024 - Jan 2025',
    bullets: [
      'Developed predictive maintenance and quality control systems using deep learning.',
      'Improved industrial planning efficiency by 15% through high-accuracy forecasting models.',
    ],
  },
]

export const projects = [
  {
    name: 'Multi-Disease Prediction System',
    tags: ['Deep Learning', 'Medical Imaging', 'Multimodal AI', 'Edge Deployment'],
    github: 'https://github.com/deoprakash/multi_disease_prediction',
    description:
      'Built a diagnostic platform for 50+ diseases achieving 93% accuracy. Reduced diagnostic error by 30% using a hybrid CNN and ensemble pipeline. Deployed Dockerized FastAPI applications with inference latency under 200ms and reduced time-to-diagnosis by 40%.',
  },
  {
    name: 'Industrial Defect Detection (Springer Nature)',
    tags: ['Computer Vision', 'CNNs', 'Research'],
    github: 'https://github.com/deoprakash/product-defect-classification',
    description:
      'Co-authored a research paper on defect identification using CNNs. Performed dataset curation, preprocessing, and augmentation using Python, Pandas, and NumPy. Evaluated models using precision and recall across multiple defect categories.',
  },
  {
    name: 'Intellixa - Multi-Agent AI Orchestration Platform',
    tags: ['LLMs', 'LangGraph', 'RAG', 'AI Agents'],
    github: 'https://github.com/deoprakash/Intellixa',
    description:
      'Implemented LangGraph-based multi-agent workflows. Reduced LLM costs by 67% and hallucinations by 30% using a Critic-Reviewer pattern. Optimized tasks into 4-6 strategic LLM calls for reliable production-grade autonomy.',
  },
  {
    name: 'Intelligent Document Assistant',
    tags: ['Generative AI', 'RAG', 'NLP'],
    github: 'https://github.com/deoprakash/QA-Bot',
    description:
      'Built an RAG-based QA system for NHAI documents to enable natural-language queries across PDF and DOCX files. Implemented ingestion, embeddings, and semantic search using FastAPI, LangChain, and vector databases, reducing manual lookup time by 80%.',
  },
]
